...
try {
  ...
  const columns = database.prepare('PRAGMA table_info(linkedin_credentials)').all();
  const requiredColumns = ['id', 'name', 'email', 'password', 'is_active', 'last_used', 'notes', 'created_at', 'updated_at'];
  ...
  const totalCount = database.prepare('SELECT COUNT(*) as count FROM linkedin_credentials').get();
  console.log(`   Total credentials: ${totalCount.count}`);

  const activeCount = database.prepare('SELECT COUNT(*) as count FROM linkedin_credentials WHERE is_active = 1').get();
  ...
  if (credentials.length === 0) {
    ...
    credentials.forEach((cred, index) => {
      ...
      const lastUsed = cred.last_used ? new Date(cred.last_used).toLocaleString() : 'Never';
  ...
  if (activeCount.count > 1) {
    ...
    console.log('   ℹ️  No active credentials (this is OK if you just added the table)\n');
  }
  ...
  hardCodedPatterns.forEach(pattern => {
    ...
    if (matches && matches.length > 0) {
      console.log(`   ⚠️  Found potential hardcoded values: ${matches.length} matches`);
  ...
  console.log(`📊 Credentials: ${totalCount.count} total, ${activeCount.count} active`);
  console.log('🔒 Security: No hardcoded credentials');
  console.log('🎯 Single-active: ' + (activeCount.count <= 1 ? 'OK' : 'WARNING'));
