...
try {
  ...
  if (columnNames.includes('retry_count') && columnNames.includes('last_retried_at')) {
  ...
  if (!columnNames.includes('retry_count')) {
    ...
    database.prepare('ALTER TABLE sites ADD COLUMN retry_count INTEGER DEFAULT 0').run();
  ...
  if (!columnNames.includes('last_retried_at')) {
    ...
    database.prepare('ALTER TABLE sites ADD COLUMN last_retried_at DATETIME').run();
    console.log('✅ last_retried_at column added');
  } else {
    console.log('ℹ️  last_retried_at column already exists');
  }

  console.log('\n✅ Migration completed successfully!\n');
  console.log('The AI retry manager can now track and retry failed analysis attempts.\n');
