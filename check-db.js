...
try {
  ...
  console.log('\n🤖 AI Status Breakdown:');
  const aiStatusBreakdown = database.prepare(`
  ...
  const pendingWithContent = database.prepare(`
  ...
  const sitesWithoutContent = database.prepare(`
    SELECT COUNT(*) as count
    FROM sites
    WHERE is_wordpress = 1
      AND (text_content IS NULL OR text_content = '')
  `).get();
  console.log(`   WordPress sites without content: ${sitesWithoutContent.count}`);

  // Show sample pending sites
  const samplePending = database.prepare(`
  ...
  if (samplePending.length > 0) {
    ...
    samplePending.forEach(site => {
      ...
      console.log(`     Status: ${site.ai_status || 'NULL'}, Content: ${site.content_length} chars`);
    });
  }

  // Check AI content relevance
  const relevanceBreakdown = database.prepare(`
  ...
  if (relevanceBreakdown.length > 0) {
    ...
    relevanceBreakdown.forEach(rel => {
      const label = rel.ai_content_relevant === null ? 'NULL' : (rel.ai_content_relevant === 1 ? 'Relevant' : 'Not Relevant');
      console.log(`   - ${label}: ${rel.count} sites`);
    });
  }

  // Show sample AI analysis for WordPress sites
  const sampleAIAnalysis = database.prepare(`
  ...
  if (sampleAIAnalysis.length > 0) {
    ...
    sampleAIAnalysis.forEach(site => {
      ...
      console.log(`     Summary: ${(site.ai_content_summary || 'N/A').substring(0, 100)}...`);
