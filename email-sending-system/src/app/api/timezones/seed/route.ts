...
export async function POST(request: Request) {
  try {
    ...
    if (countryCode) {
      if (reset) {
        return NextResponse.json({
          success: false,
          error: 'Cannot use reset mode with single country. Omit reset parameter.'
        }, { status: 400 });
      }
      ...
      if (success) {
        ...
        const seededCountry = await import('@/lib/data/country-timezones')
          .then(m => m.COUNTRY_TIMEZONES.find(
    ...
    if (reset) {
      ...
      console.warn('⚠️  RESET MODE ACTIVE - All existing data will be deleted!');
...
export async function GET() {
  return NextResponse.json({
    ...
    method: 'POST',
    description: 'Seed the country_timezones table with comprehensive timezone data',
    parameters: {
      reset: {
        type: 'boolean',
        description: 'Delete all existing data and reseed from scratch (DANGEROUS!)',
        default: false
      },
      country: {
        type: 'string',
        description: 'ISO 3166-1 alpha-2 country code to seed only that country',
        example: 'US'
      },
      verify: {
        type: 'boolean',
