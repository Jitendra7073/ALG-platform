...
export async function exportTimezoneStats(): Promise<{
  ...
  COUNTRY_TIMEZONES.forEach(country => {
    ...
    stats.businessHoursRanges[range] = (stats.businessHoursRanges[range] || 0) + 1;

    // Count timezones
    stats.timezones[country.default_timezone] = (stats.timezones[country.default_timezone] || 0) + 1;
  });
  ...
  try {
    ...
    data.forEach((row: any, index: number) => {
      if (!row.country_code || row.country_code.length !== 2) {
        result.errors.push(`Row ${index}: Invalid country_code '${row.country_code}'`);
        result.valid = false;
      }
      ...
      try {
        ...
        result.errors.push(`Row ${index}: Invalid IANA timezone '${row.default_timezone}'`);
        result.valid = false;
      }
      ...
      if (!timeRegex.test(row.business_hours_start)) {
        result.errors.push(`Row ${index}: Invalid business_hours_start '${row.business_hours_start}'`);
        result.valid = false;
      }
      if (!timeRegex.test(row.business_hours_end)) {
        result.errors.push(`Row ${index}: Invalid business_hours_end '${row.business_hours_end}'`);
        result.valid = false;
      }
      ...
      if (!Array.isArray(row.weekend_days) || row.weekend_days.length === 0) {
        result.errors.push(`Row ${index}: weekend_days must be a non-empty array`);
        result.valid = false;
      } else {
      ...
      if (row.weekend_days.length === 1 && row.weekend_days[0] !== 'Sunday') {
        result.warnings.push(`Row ${index}: Single-day weekend '${row.weekend_days[0]}' is unusual`);
      }
    });
