...
export async function seedCountryTimezones(): Promise<SeedResult> {
  ...
  console.log(`🌍 Starting country timezone seeding for ${COUNTRY_TIMEZONES.length} countries...`);
  ...
  try {
    ...
    if (!tableCheck[0]?.exists) {
      throw new Error('country_timezones table does not exist. Please run migrations first.');
    }
    ...
    for (const country of COUNTRY_TIMEZONES) {
      try {
        ...
        result.seeded++;
        console.log(`✅ Seeded: ${country.country_name} (${country.country_code})`);
      } catch (error) {
        result.failed++;
...
export async function seedSingleCountry(countryCode: string): Promise<boolean> {
  try {
    ...
    if (!country) {
      console.error(`❌ Country code '${countryCode}' not found in timezone data`);
      return false;
    }

    await upsertCountryTimezone(country);
    console.log(`✅ Seeded single country: ${country.country_name} (${country.country_code})`);
    return true;
  } catch (error) {
...
export async function verifySeedData(): Promise<{
  ...
  try {
    // Get total count
    const countResult = await executeQuery('SELECT COUNT(*) as count FROM country_timezones');
    ...
    if (missingCountries.length > 0) {
      result.valid = false;
      result.errors.push(`Missing critical countries: ${missingCountries.join(', ')}`);
    }
