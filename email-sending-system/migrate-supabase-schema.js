...
async function migrate() {
  ...
  try {
    ...
    console.log('Existing columns:', existingColumns.rows.map(col => col.column_name).join(', '));
    ...
    for (const col of siteColumnsToAdd) {
      const exists = existingColumns.rows.some(c => c.column_name === col.name);

      if (!exists) {
        console.log(`➕ Adding column: sites.${col.name}`);
        await client.query(`ALTER TABLE sites ADD COLUMN ${col.name} ${col.type}`);
    ...
    const indexesToAdd = [
      ...
      { name: 'idx_contacts_country_code', table: 'contacts', columns: 'country_code' },
    ...
    for (const idx of indexesToAdd) {
      ...
      if (!indexExists.rows[0].exists) {
        ...
        await client.query(`CREATE INDEX ${idx.name} ON ${idx.table}(${idx.columns})`);
