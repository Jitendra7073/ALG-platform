...
async function runMigration() {
  ...
  try {
    ...
    // Create indexes
    await client.query(`CREATE INDEX IF NOT EXISTS idx_country_timezones_country_code ON country_timezones(country_code);`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_country_timezones_region ON country_timezones(region);`);
    ...
    const existingCount = await client.query('SELECT COUNT(*) FROM country_timezones');
    ...
    console.log(`✅ country_timezones table created successfully with ${count.rows[0].count} countries.`);
    process.exit(0);
  } catch (error) {
