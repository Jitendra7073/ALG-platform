...
async function main() {
  ...
  try {
    ...
    const dupSenders = Object.entries(senderCounts).filter(([id, count]) => count > 1);
    ...
    const dupQueues = Object.entries(queueCounts).filter(([id, count]) => count > 1);

    console.log('📊 Analysis Results:');
    console.log(`  email_senders: ${senders?.length || 0} total, ${dupSenders.length} duplicate IDs`);
    console.log(`  email_queue: ${queues?.length || 0} total, ${dupQueues.length} duplicate IDs`);
    ...
    if (dupSenders.length > 0) {
      console.log('-- Clean up duplicate email_senders (keep earliest created)');
