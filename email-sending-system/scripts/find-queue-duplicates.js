  ...
  try {
    ...
    if (duplicateIds.size > 0) {
      ...
      for (const dupId of duplicateIds) {
        ...
        dupItems.forEach((item, idx) => {
          console.log(`  [${idx + 1}] Status: ${item.status}, Recipient: ${item.recipient_email}, Created: ${item.created_at}`);
        });
        console.log('');
      }
      ...
      for (const dupId of duplicateIds) {
        const dupItems = allQueues.filter(q => q.id === dupId);
        const sorted = [...dupItems].sort((a, b) =>
          new Date(a.created_at).getTime() - new Date(b.created_at).getTime()
        );

        console.log(`-- ID: ${dupId} - Keep ${sorted[0].created_at}, delete ${sorted.length - 1} newer`);
        for (let i = 1; i < sorted.length; i++) {
          console.log(`DELETE FROM email_queue WHERE id = '${dupId}' AND created_at = '${sorted[i].created_at}';`);
        }
        console.log('');
      }
    ...
    if (problemItems.length > 1) {
      ...
      problemItems.forEach((item, i) => {
        console.log(`   [${i + 1}] Status: ${item.status}, Created: ${item.created_at}`);
      });
    }
