...
async function checkDuplicateSenders() {
  ...
  try {
    ...
    if (duplicates.length > 0) {
      ...
      console.log('2. Manually remove duplicate records keeping only one per ID');
...
HAVING COUNT(*) > 1;
    ...
    if (problemSender.length > 1) {
      ...
      problemSender.forEach((s, i) => {
        console.log(`   ${i + 1}. Email: ${s.email}, Name: ${s.name}, Created: ${s.created_at}`);
      });
    } else if (problemSender.length === 0) {
      console.log('   ⚠️ SENDER NOT FOUND! The sender_id in the queue doesn\'t exist.');
    } else {
