...
async function diagnose() {
  ...
  try {
    ...
    console.log(`✅ Eligible emails to process: ${eligibleEmails?.length || 0}`);
    ...
    if (senders && senders.length > 0) {
      ...
      senders.forEach((sender, idx) => {
        ...
        console.log(`       Has App Password: ${sender.app_password ? '✅' : '❌'}`);
      });

      const totalCapacity = senders.reduce((sum, s) => sum + (s.daily_limit - s.sent_today), 0);
      console.log(`\n   📊 Total remaining capacity: ${totalCapacity} emails`);
    } else {
      console.log('   ⚠️ No active senders found! Add senders in the Senders page.');
    }
    ...
    if (recentSends && recentSends.length > 0) {
      ...
      recentSends.forEach((log, idx) => {
        ...
        console.log(`   [${idx + 1}] ${time} - ${log.contact_email} (${log.status})`);
      });
    }
    ...
    const hasCapacity = senders && senders.some(s => (s.daily_limit - s.sent_today) > 0);
    ...
    if (hasEligibleEmails && hasActiveSenders && hasCapacity) {
      ...
      console.log('   Go to http://localhost:3000/history and click "Start Queue for Today"');
