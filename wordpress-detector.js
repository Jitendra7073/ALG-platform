...
async function checkWordPress(page, url) {
  ...
  const CONFIDENCE_THRESHOLDS = {
    ...
    LOW: 2, // Some evidence
    MINIMUM: 7, // Current threshold for positive detection - increased to 7 to avoid PHP false positives
  };
      ...
      if (wpJsonResponse) {
        ...
        if (
          ...
          try {
            ...
            addIndicator("WordPress REST API endpoint (unvalidated)", 0.5);
          }
        }
      }
    } catch (e) {
 ...
 */
async function googleSearch(page, query, maxResults = 10, countrySettings = null) {
  console.log(`\n🔍 Searching Google for: "${query}"\n`);

  const domain = countrySettings ? countrySettings.domain : "google.com";
  const gl = countrySettings ? countrySettings.gl : "us";
  const hl = countrySettings ? countrySettings.hl : "en";

  // Navigate to Google
  const searchUrl = `https://www.${domain}/search?q=${encodeURIComponent(query)}&gl=${gl}&hl=${hl}`;
...
async function main() {
  ...
  const context = await chromium.launchPersistentContext(userDataDir, {
    ...
    locale: "en-US", // Default, will be overridden by args in real usage if needed
  ...
  try {
    // Get search query and optional country from command line arguments
    const searchQuery = process.argv[2] || "myntra coupon code";
    const countryArg = process.argv[3] || "in";

    // Regional settings mapping (same as in server.js)
    const regionalSettings = {
      'in': { gl: 'in', hl: 'en', locale: 'en-IN', timezoneId: 'Asia/Kolkata', domain: 'google.co.in' },
      'us': { gl: 'us', hl: 'en', locale: 'en-US', timezoneId: 'America/New_York', domain: 'google.com' },
      'uk': { gl: 'uk', hl: 'en', locale: 'en-GB', timezoneId: 'Europe/London', domain: 'google.co.uk' },
      'ca': { gl: 'ca', hl: 'en', locale: 'en-CA', timezoneId: 'America/Toronto', domain: 'google.ca' },
      'au': { gl: 'au', hl: 'en', locale: 'en-AU', timezoneId: 'Australia/Sydney', domain: 'google.com.au' },
      'de': { gl: 'de', hl: 'de', locale: 'de-DE', timezoneId: 'Europe/Berlin', domain: 'google.de' },
      'fr': { gl: 'fr', hl: 'fr', locale: 'fr-FR', timezoneId: 'Europe/Paris', domain: 'google.fr' },
      'ae': { gl: 'ae', hl: 'en', locale: 'en-AE', timezoneId: 'Asia/Dubai', domain: 'google.ae' },
      'sg': { gl: 'sg', hl: 'en', locale: 'en-SG', timezoneId: 'Asia/Singapore', domain: 'google.com.sg' },
    };

    const settings = regionalSettings[countryArg.toLowerCase()] || regionalSettings['in'];

    // Update context with regional settings
    await context.addCookies([]); // Placeholder for potential cookie injection
    // Note: relaunching context would be cleaner if we wanted full locale change, 
    // but for standalone script we just set it at launch below.

    // Perform Google search
    const urls = await googleSearch(page, searchQuery, 10, settings);
