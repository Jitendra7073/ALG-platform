...
router.put('/:id', (req, res) => {
  try {
    ...
    const existing = db.get('SELECT * FROM linkedin_credentials WHERE id = ?', [id]);
    if (!existing) {
      return res.status(404).json({ success: false, error: 'Credential not found' });
    }
...
router.post('/:id/set-active', (req, res) => {
  try {
    ...
    db.run('UPDATE linkedin_credentials SET is_active = 1, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [id]);
