...
class LinkedInCompanyScraper {
  ...
  async init() {
    ...
    if (
      ...
      throw new Error(
        "No active LinkedIn credential found. Please add credentials in the admin panel.",
      );
    }

    console.log(
      `📋 Using credential: ${activeCredential.name} (${activeCredential.email})`,
    );
  ...
  async loginToLinkedIn(credential) {
    try {
      ...
      try {
        ...
        console.log(
          "⚠️  domcontentloaded timeout, trying with 'load' strategy...",
        );
        await this.page.goto("https://www.linkedin.com/login", {
          waitUntil: "load",
          timeout: 60000,
      ...
      inputInfo.inputs.forEach((input, i) => {
        console.log(
          `      Input ${i + 1}: type="${input.type}", id="${input.id}", visible=${input.visible}`,
        );
      });
      ...
      inputInfo.buttons.forEach((btn, i) => {
        if (btn.text)
          console.log(
            `      Button ${i + 1}: type="${btn.type}", text="${btn.text}"`,
          );
      });
      ...
      for (const input of allInputs) {
        ...
        if (visible && !emailField) {
          if (type === "email" || (type === "text" && inputId === "username")) {
            emailField = input;
            console.log(
              `   ✓ Found email field: type="${type}", id="${inputId}"`,
            );
          }
        }
      ...
      if (!emailField || !passwordField) {
        ...
        console.log(
          `      Password field found: ${passwordField ? "YES" : "NO"}`,
        );
        await this.page.screenshot({ path: "linkedin-no-form-fields.png" });
        throw new Error(
          "Login form not found. LinkedIn may have changed their page layout or you need to complete verification manually.",
        );
      }
      ...
      try {
        ...
        await this.page.screenshot({
          path: "linkedin-fill-password-error.png",
        });
        throw fillError;
      }
      ...
      try {
        ...
        for (let i = 0; i < buttons.length; i++) {
          const buttonText = await buttons[i].evaluate(
            (el) => el.textContent || "",
          );
          const trimmedText = buttonText.trim();
          const buttonClass = await buttons[i].evaluate(
            (el) => el.className || "",
          );
          const buttonType = await buttons[i].evaluate(
            (el) => el.getAttribute("type") || "",
          );

          if (trimmedText) {
            console.log(
              `      Button ${i + 1}: "${trimmedText}" (type="${buttonType}")`,
            );
            console.log(
              `         Class snippet: ${buttonClass.substring(0, 50)}...`,
            );
          }
        }
        ...
        for (let i = 0; i < buttons.length; i++) {
          ...
          if (trimmedText === "Sign in") {
            console.log(
              `      → Found button ${i + 1} with exact text: "${trimmedText}"`,
            );
            ...
            try {
              ...
              try {
                ...
                try {
                  ...
                  console.log(
                    `      ⚠️  Focus + Enter failed: ${err3.message}`,
                  );
                }
              }
            }
        ...
        if (!buttonClicked) {
          ...
          for (let i = 0; i < buttons.length; i++) {
            try {
              const innerText = await buttons[i].evaluate(
                (el) => el.innerText || "",
              );
        ...
        if (!buttonClicked) {
          ...
          try {
            const xpathButton = await this.page.waitForSelector(
              "//button[contains(., 'Sign in') and not(contains(., 'Sign in with'))]",
              {
                timeout: 5000,
              },
            );
        ...
        if (!buttonClicked) {
          ...
          if (buttonIndex >= 0) {
            console.log(
              `      → Found button at index ${buttonIndex} in browser context`,
            );
        ...
        if (!buttonClicked) {
          ...
          throw new Error(
            "Failed to click Sign in button after trying all strategies",
          );
        }
      } catch (buttonError) {
        ...
        await this.page.screenshot({
          path: "linkedin-button-error.png",
          fullPage: true,
        });
        throw buttonError;
      }
      ...
      if (
        ...
        console.log("  You have 5 minutes to complete verification\n");
        ...
        try {
          ...
          if (
            !finalCheckUrl.includes("login") &&
            !finalCheckUrl.includes("checkpoint")
          ) {
            console.log(
              " Verification appears successful (URL changed). Continuing...\n",
            );
          } else {
            throw new Error(
              "Verification timed out after 5 minutes. Please complete manual verification and try again.",
            );
          }
        }
        ...
        throw new Error(
          "Login failed. Please check your email and password. If credentials are correct, LinkedIn may require additional verification.",
        );
      } else {
  ...
  async scrapeCompanyPage(companyUrl, siteId) {
    try {
      ...
      console.log(
        ` Selected ${executives.length} structured executives (3 Founders + 1 CEO + 1 CTO)`,
      );
  ...
  selectStructuredExecutives(allExecutives) {
    ...
    for (const role of founderPriority) {
      ...
      for (const match of matches) {
        ...
        founders.push(match);
        console.log(
          `   🏗️ Founder ${founders.length}: ${match.name || "Unknown"} (${match.roleCategory})`,
        );
      }
    }
    ...
    console.log(
      `    Structure: ${founders.length} Founder(s), ${ceo ? 1 : 0} CEO, ${cto ? 1 : 0} CTO`,
    );
      ...
      allCards.forEach((card, index) => {
        try {
          ...
          if (hasTargetKeyword) {
            ...
            console.log(`[LinkedIn Scraper]  Added: ${name} (${roleCategory})`);
          }
        } catch (e) {
