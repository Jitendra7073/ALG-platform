...
async function main() {
  ...
  try {
    ...
    const settings =
      regionalSettings[countryArg.toLowerCase()] || regionalSettings["in"];
    ...
    const ignoredUrls = [];
    const ignoredTags = db
      .getAllIgnoredTags()
      .filter((t) => t.scope === "url" || !t.scope);
    ...
    if (newUrls.length === 0) {
      console.log(
        "All URLs already exist in database or are excluded/ignored. Nothing to scrape.",
      );
