...
async function runDiagnostics() {
  ...
  if (senders.length === 0) {
    ...
    if (activeSenders.length === 0) {
      ...
      activeSenders.forEach((sender) => {
        ...
        const needsReset = sender.last_reset_date ? new Date(sender.last_reset_date).toDateString() !== today : true;
  ...
  if (queueStats.queued > 0) {
    ...
    const queuedBreakdown = await db.get(`
      SELECT
        COUNT(*) as total,
        COUNT(CASE WHEN scheduled_at IS NULL THEN 1 END) as immediate,
        COUNT(CASE WHEN scheduled_at IS NOT NULL AND scheduled_at <= NOW() THEN 1 END) as ready,
        COUNT(CASE WHEN scheduled_at IS NOT NULL AND scheduled_at > NOW() THEN 1 END) as scheduled
      FROM email_queue
      WHERE status = 'queued'
    `);
    ...
    if (queuedBreakdown.scheduled > 0) {
      const nextScheduled = await db.get(`
    ...
    const firstQueued = await db.all(`
  ...
  const stuckEmails = await db.all(`
  ...
  const templates = await db.all("SELECT * FROM email_templates WHERE is_active = true");
  ...
  const settings = await db.all("SELECT * FROM email_settings");
  const settingMap = {};
  settings.forEach((s) => {
  ...
  if (issuesFound === 0 && warnings === 0) {
    ...
    console.log("\n🔧 Recommended actions:");
    if (senders.filter((s) => s.is_active === true).length === 0) {
      console.log("   1. Activate at least one email sender account");
    }
    if (queueStats.queued > 0 && senders.filter((s) => s.is_active === true && s.sent_today < s.daily_limit).length === 0) {
