...
try {
  ...
  recentSites.forEach((site) => {
    const relevant =
      site.ai_content_relevant === 1 ? " Relevant" : "⚠️  Not Relevant";
  ...
  const nullSearchQueries = database
    .prepare(
      `
  ...
  console.log(
    `   Sites with NULL/empty search_query: ${nullSearchQueries.count}`,
  );

  // Get examples of sites with NULL search_query
  const nullExamples = database
    .prepare(
      `
  ...
  if (nullExamples.length > 0) {
    ...
    nullExamples.forEach((site) => {
      ...
      console.log(
        `      [${site.id}] ${relevant} ${site.url} - Category: ${site.ai_actual_category || "N/A"}`,
      );
    });
  }
  ...
  console.log(
    `   Average content length: ${Math.round(contentStats.avg_length)} characters`,
  );
  ...
  categories.forEach((cat) => {
    ...
    console.log(
      `   ${cat.ai_actual_category}: ${cat.count} (${cat.relevant_count} relevant - ${percentage}%)`,
    );
  });

  // Relevance rate
  console.log("\n5. OVERALL RELEVANCE RATE:\n");
  const relevanceStats = database
    .prepare(
      `
    SELECT
      COUNT(*) as total,
  ...
  const relevanceRate = Math.round(
    (relevanceStats.relevant / relevanceStats.total) * 100,
  );
  console.log(`   Total processed: ${relevanceStats.total}`);
  console.log(
    `   Content relevant: ${relevanceStats.relevant} (${relevanceRate}%)`,
  );
  console.log(
    `   Content not relevant: ${relevanceStats.total - relevanceStats.relevant} (${100 - relevanceRate}%)`,
  );
