...
function getStats() {
  ...
  try {
    stats.searches =
      db.prepare("SELECT COUNT(*) as count FROM searches").get()?.count || 0;
    stats.sites =
      db.prepare("SELECT COUNT(*) as count FROM sites").get()?.count || 0;
    stats.wordpressSites =
      db
        .prepare("SELECT COUNT(*) as count FROM sites WHERE is_wordpress = 1")
        .get()?.count || 0;
    stats.contacts =
      db.prepare("SELECT COUNT(*) as count FROM contacts").get()?.count || 0;
    stats.keywords =
      db.prepare("SELECT COUNT(*) as count FROM keywords").get()?.count || 0;
    stats.executives =
      db.prepare("SELECT COUNT(*) as count FROM company_executives").get()
        ?.count || 0;

    // Email system tables (may not exist)
    try {
      stats.emailSenders =
        db.prepare("SELECT COUNT(*) as count FROM email_senders").get()
          ?.count || 0;
      stats.emailTemplates =
        db.prepare("SELECT COUNT(*) as count FROM email_templates").get()
          ?.count || 0;
      stats.emailCampaigns =
        db.prepare("SELECT COUNT(*) as count FROM email_campaigns").get()
          ?.count || 0;
      stats.emailQueue =
        db.prepare("SELECT COUNT(*) as count FROM email_queue").get()?.count ||
        0;
    } catch (e) {
...
function clearLeadsData() {
  ...
  try {
    ...
    db.exec(
      "DELETE FROM sqlite_sequence WHERE name IN ('company_executives', 'contacts', 'sites', 'searches', 'email_send_log')",
    );
    ...
    rl.question(
      ...
      async (answer) => {
        ...
        switch (answer.trim()) {
          ...
          case "5":
            log(
              "\n⚠️  WARNING: This will completely delete the database file!",
              "red",
            );
