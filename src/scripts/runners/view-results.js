...
function showAllSearches() {
  ...
  searches.forEach((search) => {
    const percentage = (
      (search.wordpress_count / search.total_sites) *
      100
    ).toFixed(1);
    console.log(`\nID: ${search.id}`);
    ...
    console.log(
      `Results: ${search.total_sites} sites | WordPress: ${search.wordpress_count} (${percentage}%) | Non-WordPress: ${search.non_wordpress_count}`,
    );
    console.log("─".repeat(100));
  });
...
function showAllWordpressSites() {
  ...
  uniqueSites.forEach((site) => {
    ...
    console.log(
      `  Last checked: ${new Date(site.checked_at).toLocaleString()}`,
    );
  });
...
function showStatistics() {
  ...
  const wpPercentage =
    stats.total_sites_checked > 0
      ? (
          (stats.total_wordpress_sites / stats.total_sites_checked) *
          100
        ).toFixed(1)
      : 0;
  ...
  console.log(
    `WordPress sites found: ${stats.total_wordpress_sites || 0} (${wpPercentage}%)`,
  );
  ...
  if (stats.topQueries && stats.topQueries.length > 0) {
    ...
    stats.topQueries.forEach((q, i) => {
      console.log(`\n  ${i + 1}. "${q.query}"`);
      console.log(
        `     Searched ${q.search_count} time(s), found ${q.wordpress_found} WordPress sites`,
      );
    });
  }
...
function showLinkedinProfiles() {
  ...
  profiles.linkedin_profiles.forEach((profile) => {
    ...
    console.log(
      `  Extracted: ${new Date(profile.created_at).toLocaleString()}`,
    );
  });
...
function showContacts(type = "all") {
  ...
  console.log("\n" + "═".repeat(100));
  console.log(
    `📞 ALL ${type.toUpperCase()} CONTACTS (${contacts.total} total)`,
  );
  console.log("═".repeat(100));
  ...
  contacts.contacts.forEach((contact) => {
    ...
    console.log(
      `  Extracted: ${new Date(contact.created_at).toLocaleString()}`,
    );
  });
