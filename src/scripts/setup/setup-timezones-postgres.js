/**
 * Setup country_timezones table for PostgreSQL
 * Creates the table and populates it with timezone data for common countries
 *
 * Usage: node src/scripts/setup/setup-timezones-postgres.js
 */
...
// Insert data using individual INSERT statements (async)
async function insertData() {
  let inserted = 0;
  for (const country of timezoneData) {
    try {
      await db.run(`
      ...
      console.error(`   ❌ Error inserting ${country.code}:`, e.message);
    }
  }
  return inserted;
}

insertData().then(inserted => {
  console.log(`✅ Inserted ${inserted} countries with timezone data`);

  console.log("\n📊 Sample data:");
  db.all(`
    SELECT country_code, name, timezone, offset_hours, business_start, business_end
    FROM country_timezones
    LIMIT 5
  `).then(sample => {
    sample.forEach((row) => {
      console.log(`   ${row.country_code.toUpperCase()}: ${row.name} (${row.timezone}, UTC${row.offset_hours >= 0 ? '+' : ''}${row.offset_hours}) - Business: ${row.business_start}:00-${row.business_end}:00`);
    });
