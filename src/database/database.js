...
function initDatabase() {
  ...
  try {
    db.prepare(
      "UPDATE sites SET ai_status = 'pending' WHERE ai_status = 'processing'",
    ).run();
  } catch (e) {}
...
function deleteKeyword(id) {
  ...
  try {
    ...
    const keyword = db
      .prepare("SELECT keyword FROM keywords WHERE id = ?")
      .get(id);

    if (keyword) {
      // Find all searches that match this keyword
      const searches = db
        .prepare("SELECT id FROM searches WHERE query = ?")
        .all(keyword.keyword);
      ...
      if (searchIds.length > 0) {
        ...
        const sites = db
          .prepare(`SELECT id FROM sites WHERE search_id IN (${placeholders})`)
          .all(...searchIds);
        ...
        if (siteIds.length > 0) {
          ...
          db.prepare(
            `DELETE FROM contacts WHERE site_id IN (${sitePlaceholders})`,
          ).run(...siteIds);
          // Delete executives for those sites
          db.prepare(
            `DELETE FROM company_executives WHERE site_id IN (${sitePlaceholders})`,
          ).run(...siteIds);
        }

        // Delete sites for those searches
        db.prepare(
          `DELETE FROM sites WHERE search_id IN (${placeholders})`,
        ).run(...searchIds);
        // Delete the searches themselves
        db.prepare(`DELETE FROM searches WHERE id IN (${placeholders})`).run(
          ...searchIds,
        );
      }
    }
...
function getIgnoredTagById(id) {
  const db = initDatabase();
  const tag = db.prepare(`SELECT * FROM ignored_tags WHERE id = ?`).get(id);
  db.close();
  return tag || null;
}
...
function deleteIgnoredTag(id) {
  ...
  db.exec("PRAGMA foreign_keys = OFF");
  try {
    const result = db.prepare(`DELETE FROM ignored_tags WHERE id = ?`).run(id);
    return result.changes > 0;
  } finally {
    db.exec("PRAGMA foreign_keys = ON");
    db.close();
...
function bulkDeleteIgnoredTags(ids) {
  ...
  try {
    ...
    const result = db
      .prepare(`DELETE FROM ignored_tags WHERE id IN (${placeholders})`)
      .run(...ids);
    return result.changes;
  } finally {
    db.close();
...
function getAllSites(
  ...
  else if (filter === "content-relevant")
    whereClauses.push("ai_content_relevant = 1");
  else if (filter === "content-mismatch")
    whereClauses.push("ai_content_relevant = 0");
  else if (filter === "ai-pending")
    whereClauses.push(
      "is_wordpress = 1 AND (ai_status = 'pending' OR ai_status IS NULL)",
    );
...
function getStructuredExecutives(page = 1, limit = 20, search = null) {
  ...
  if (search && search.trim() !== "") {
    whereClause =
      "(ce.company_name LIKE ? OR ce.name LIKE ? OR ce.headline LIKE ?)";
    params.push(`%${search}%`, `%${search}%`, `%${search}%`);
  }
  ...
  const structured = companies.map((company) => {
    ...
    const founders = execs
      .filter((e) => founderRoles.includes(e.role_category))
      .slice(0, 3);
...
function bulkDeleteSites(ids) {
  ...
  try {
    const placeholders = ids.map(() => "?").join(",");
    // Manually delete related data to ensure cascade
    db.prepare(`DELETE FROM contacts WHERE site_id IN (${placeholders})`).run(
      ...ids,
    );
    db.prepare(
...
function getSiteDeletionPreview(ids) {
  if (!ids || ids.length === 0)
    return { sites: 0, emails: 0, phones: 0, linkedins: 0, executives: 0 };
  const db = initDatabase();
  const placeholders = ids.map(() => "?").join(",");
  ...
  const phones = db
    .prepare(
      `SELECT COUNT(*) as count FROM contacts WHERE site_id IN (${placeholders}) AND type = 'phone'`,
    )
    .get(...ids).count;
  const linkedins = db
    .prepare(
      `SELECT COUNT(*) as count FROM contacts WHERE site_id IN (${placeholders}) AND type = 'linkedin'`,
    )
    .get(...ids).count;
  const executives = db
    .prepare(
      `SELECT COUNT(*) as count FROM company_executives WHERE site_id IN (${placeholders})`,
    )
    .get(...ids).count;
...
function deleteAllData() {
  ...
  try {
    ...
    try {
      counts.email_send_log = db
        .prepare("DELETE FROM email_send_log")
        .run().changes;
    } catch (e) {
      counts.email_send_log = 0;
    }
    try {
      counts.email_queue = db.prepare("DELETE FROM email_queue").run().changes;
    } catch (e) {
      counts.email_queue = 0;
    }
    try {
      counts.email_campaigns = db
        .prepare("DELETE FROM email_campaigns")
        .run().changes;
    } catch (e) {
      counts.email_campaigns = 0;
    }
    try {
      counts.email_templates = db
        .prepare("DELETE FROM email_templates")
        .run().changes;
    } catch (e) {
      counts.email_templates = 0;
    }
    try {
      counts.email_senders = db
        .prepare("DELETE FROM email_senders")
        .run().changes;
    } catch (e) {
      counts.email_senders = 0;
    }

    // Core data tables (order matters for FK)
    counts.company_executives = db
      .prepare("DELETE FROM company_executives")
      .run().changes;
    ...
    counts.excluded_domains = db
      .prepare("DELETE FROM excluded_domains")
      .run().changes;
